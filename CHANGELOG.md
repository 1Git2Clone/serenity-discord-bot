...
- `attachments` was recorded with `%`, which made it a string in the trace backend, so `sum_over_time(span.attachments)` returned nothing — silently, since an unsummable attribute yields an empty series rather than an error. Counts are now recorded without a sigil and arrive as integers
