...
- `attachment_urls` span field on `handle_message` — space-separated CDN links for the message's attachments. Discord signs these and they expire in roughly a day, so this answers "what was just posted" and cannot be an archive; the bot stores no attachment bytes
- `content` span field on `handle_message` — the message text, so a traces-backed dashboard can show what was said instead of a row of ids and counts. A deliberate widening of the no-content rule, which was aimed at whole-struct dumps; `levenshtein_core` has recorded a lowercased copy of every message all along
- `guild_name`, `author_name` and `links` span fields on `handle_message` — dashboards grouped on snowflakes were lists of numbers nobody can read; the names make a guild leaderboard and a most-active-users table legible without a second lookup. `author_name` is the unique Discord username, not the per-guild display name, so it is the same value everywhere. `guild_name` falls back to `DM` with no guild and to the snowflake when the cache has not seen the guild yet
...
- `attachments` was recorded with `%`, which made it a string in the trace backend, so `sum_over_time(span.attachments)` returned nothing — silently, since an unsummable attribute yields an empty series rather than an error. Counts are now recorded as `i64` without a sigil. `i64` and not `u64`: tracing-opentelemetry's span visitor has no `record_u64`, so a `u64` falls through `Visit`'s default to `record_debug` and arrives as a string anyway
